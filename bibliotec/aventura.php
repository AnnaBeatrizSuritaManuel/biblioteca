...
<head>
    ...
    <title><?= htmlspecialchars(ucfirst($genero)) ?> - Bibliotec</title>
    ...
    <section class="section">
        <div class="container">
            <div class="section-header">
                <h2 class="section-title">Livros de <?= htmlspecialchars(ucfirst($genero)) ?></h2>
                <p class="section-subtitle">Jornadas emocionantes, explorações e descobertas incríveis</p>
            </div>
            
            <?php if(empty($livros)): ?>
                <div class="text-center" style="padding: 3rem;">
                    <i class="fas fa-mountain" style="font-size: 3rem; color: var(--text-muted); margin-bottom: 1rem;"></i>
                    <p style="color: var(--text-secondary);">Nenhum livro de <?= htmlspecialchars($genero) ?> cadastrado ainda.</p>
                    <?php if(isset($_SESSION['usuario_tipo']) && $_SESSION['usuario_tipo'] == 'admin'): ?>
                        <a href="admin/livros.php" class="btn btn-primary mt-2">Cadastrar Livros de <?= htmlspecialchars($genero) ?></a>
                    <?php endif; ?>
                </div>
            <?php else: ?>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <p class="card-text">
                                    ...
                                    <small>• <?= htmlspecialchars($livro['ano_publicado']) ?></small>
                                </p>
                                <div class="card-actions">
                                    <button class="btn btn-primary btn-small" onclick="openBookModal(<?= (int)$livro['id_livro'] ?>)">
                                        Ver Detalhes
                                    </button>
                                    <?php if(isset($_SESSION['usuario_id'])): ?>
                                        <button class="btn btn-outline btn-small favorite-btn" data-livro="<?= (int)$livro['id_livro'] ?>">
                                            <i class="far fa-heart"></i>
                                        </button>
                                        <button class="btn btn-outline btn-small cart-btn" data-livro="<?= (int)$livro['id_livro'] ?>">
    ...
    <section class="section" style="background: var(--background);">
        <div class="container">
            <div class="grid grid-2">
                <div>
                    <h3>Sobre o Gênero <?= htmlspecialchars(ucfirst($genero)) ?></h3>
                    <p>O gênero de aventura transporta o leitor para jornadas emocionantes, explorações de lugares desconhecidos e enfrentamento de desafios extraordinários. São histórias que inspiram coragem e curiosidade.</p>
                    ...
                    <ul>
                        ...
                        <li>Personagens corajosos e determinados</li>
                    </ul>
                </div>
                <div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <h4>Jules Verne</h4>
                            <p>Pioneiro da ficção científica e aventura, autor de "Volta ao Mundo em 80 Dias".</p>
                        </div>
                    </div>
                    <div class="card mt-2">
                        <div class="card-body">
                            <h4>Robert Louis Stevenson</h4>
                            <p>Autor de "A Ilha do Tesouro", clássico da literatura de aventura.</p>
                        </div>
                    </div>
                    <div class="card mt-2">
                        <div class="card-body">
                            <h4>Jack London</h4>
                            <p>Mestre das histórias de sobrevivência e aventura na natureza selvagem.</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    function openBookModal(livroId) {
        const modalContent = `
            <div style="text-align: center; padding: 1rem;">
                <i class="fas fa-mountain" style="font-size: 4rem; color: var(--primary-main); margin-bottom: 1rem;"></i>
                <h4>Detalhes do Livro de ${<?= json_encode(htmlspecialchars($genero)) ?>}</h4>
