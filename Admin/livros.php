...
if (isset($_POST['excluir_livro'])) {
    ...
    try {
        ...
        $stmt_emprestimos = $pdo->prepare("DELETE FROM EMPRESTIMO WHERE id_livro = ?");
        $stmt_emprestimos->execute([$id_livro]);
        
        $stmt_escrito = $pdo->prepare("DELETE FROM ESCRITO WHERE id_livro = ?");
        $stmt_escrito->execute([$id_livro]);
        
        $stmt_comentarios = $pdo->prepare("DELETE FROM comentarios_livros WHERE id_livro = ?");
        $stmt_comentarios->execute([$id_livro]);
        
        $stmt_favoritos = $pdo->prepare("DELETE FROM favoritos WHERE id_livro = ?");
        $stmt_favoritos->execute([$id_livro]);
        
        $stmt_carrinho = $pdo->prepare("DELETE FROM carrinho WHERE id_livro = ?");
        $stmt_carrinho->execute([$id_livro]);
        ...
        $pdo->rollBack();
        $_SESSION['erro'] = "Erro ao excluir livro: " . $e->getMessage();
    }
